...
# Write the README
with open("README.md", "w", encoding="utf8", newline="\r\n") as f:
    f.write(header)
    ...
    results = sorted(
        results,
        key=lambda result: lang_stats[result[0]["code"]]["missing"],
        reverse=True,
    )
    for lang, percentage in results:
        ...
        comment = (
            "(100% complete! 🎉)"
            if stats["missing"] == 0
            else f"({percentage}% complete; {stats['missing']} remaining)"
        )
        f.write(f"* {flag} [{lang_name}](/translations/{lang_code}.po) {comment}\n")

    date = datetime.datetime.now(datetime.UTC)
    f.write(f"\n> Report generated on {date.strftime('%b %d %Y %H:%M:%S')} UTC\n")
